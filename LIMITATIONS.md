...
Approvals are exact: workload UID, resolved executable identity and complete argv digest must all match at the protected pre-exec `start` gate. Changing any argument requires a new approval. A direct or third-party launch cannot consume an approval, and descendants or siblings do not inherit one. Revoking an approval affects future protected launches, not a workload already running under that approval.
