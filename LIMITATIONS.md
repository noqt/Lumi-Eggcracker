...
Lumi Eggcracker 1.0.10 autonomously acts on a complete match from its four active,
...
result or CI pass does not qualify a 1.0.10 package or another host.

Ollama and vLLM are active only for the exact native CPU topology fixtures and
launcher identities in the qualified release catalogue. Name-only profiles for TGI,
...
Selected workloads are offline-only in 1.0.10. Loopback is allowed; all
