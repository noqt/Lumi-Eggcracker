...
class ApprovalTests(unittest.TestCase):
    def test_exact_approval_matches_only_exact_uid_and_argv(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            binary.chmod(0o755)
            record = create(root, name="qwen", uid=1001, argv=[str(binary), "-m", "/models/qwen.gguf"], operator_uid=1001)
    ...
    def test_revoke_removes_exact_record(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            create(root, name="qwen", uid=1001, argv=[str(binary)], operator_uid=1001)
