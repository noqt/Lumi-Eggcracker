...
or canary gates. It retains the 1.0.9 runtime repair, which keeps an exact
admitted workload active when its launch
...
The in-place upgrader accepts the immediately preceding 1.0.9 candidate.
...
Approval-sensitive launchers can now request fail-closed admission with
`eggcracker start --require-approval`. If the exact root approval is absent or
concurrently revoked, the supervisor rejects the request while holding the
serialized approval/start locks and before creating a namespace, gate, cgroup
or workload. Ordinary unapproved starts remain available so autonomous
recognition and containment continue to work as designed.

Autonomous recognition now reclaims the exact offline namespace pair for an
Eggcracker-owned run immediately after its terminal state and detection receipt
are durable. Cleanup failure does not alter the proven kill, but it fails health
closed and leaves the retained run identity available for restart recovery.
...
After exact offline-boundary teardown succeeds, terminal-history pruning now
runs again with the just-completed run protected. This keeps the bounded store
at 128 records even when namespace identity reuse made older records
temporarily non-removable during the pre-cleanup persistence pass. An uncertain
boundary remains retained and still fails health closed.

The bounded incident store now keeps its last fully validated contents in
memory. Exact record metadata is checked before every use, any change forces a
complete integrity validation, and successful root-owned atomic writes update
the cache directly. This removes repeated parsing and hashing of 256 incident
...
The public release is bound to the exact qualified commit, artifact hashes,
disposable Ubuntu qualification evidence and the internal
black-box/source-informed campaign. Its annotated tag and detached checksum
list are signed by the pinned Eggcracker release key. Unsupported formats,
runtimes, containers, remote workloads, behavioural recognition, host-wide
isolation and remote retaliation remain outside the claim.
